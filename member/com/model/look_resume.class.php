...
class look_resume_controller extends company{
	function index_action(){
		...
		if(is_array($rows)){
			...
			$resume=$this->obj->DB_select_alls("resume","resume_expect","a.id=b.id and b.`id` in (".pylode(",",$resume_id).")","a.`name`,a.`sex`,a.`exp`,a.`edu`,a.`nametype`,b.`id`,b.`minsalary`,b.`maxsalary`,b.job_classid,b.`uname`,b.`resume_id`");
	...
	function del_action(){
		if($_POST['delid']||$_GET['id']){
            ...
            $nid=$this->obj->DB_delete_all("look_resume","`id` in (".$delid.") and `com_id`='".$this->uid."'");
