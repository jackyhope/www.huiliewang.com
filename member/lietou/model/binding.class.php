...
class binding_controller extends lietou{
	...
	function save_action(){
		...
		if($_POST['moblie']){
			$row=$this->obj->DB_select_once("lietou_cert","`uid`='".$this->uid."' and `check`='".$_POST['moblie']."' and `type`='2'");
			if(!empty($row)){
				...
				$this->obj->DB_update_all("lietou_cert","`status`='1'","`uid`='".$this->uid."' and `check2`='".$_POST['code']."'");
				$this->obj->member_log("手机绑定");
				$pay=$this->obj->DB_select_once("lietou_pay","`pay_remark`='手机绑定' and `com_id`='".$this->uid."'");
				if(empty($pay)){
	    ...
	    if($_POST['upfile']){
		    $row=$this->obj->DB_select_once("lietou_cert","`uid`='".$this->uid."' and type='3'");
			...
			$this->obj->DB_update_all("lietou","`name`='".$company_name."',`yyzz_status`='".$sql['status']."'","`uid`='".$this->uid."'");
			$sql['step']=1;
			...
			if(is_array($row)){
				$where['uid']=$this->uid;
				$where['type']='3';
				$id=$this->obj->update_once("lietou_cert",$sql,$where);
				$this->obj->member_log("更新营业执照");
			}else{
				$sql['uid']=$this->uid;
				$sql['did']=$this->userdid;
				$sql['type']=3;
				$id=$this->obj->insert_into("lietou_cert",$sql);
