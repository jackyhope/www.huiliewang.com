...
//			}
			if(!$this->uid || !$this->username){
				...
				if($this->config['sy_msg_cert']=="2" && 1<>1){
					echo 3;die;
				}else{
...
//		}
			...
			if(is_array($city_type[$_POST['id']])){
				...
				foreach($city_type[$_POST['id']] as $v){
					if($_POST['gettype']=="citys"){
                        ...
                        $data.='<li><a href="javascript:;" onclick="select_city(\''.$v.'\',\'comcitys\',\''.$city_name[$v].'\',\'three_city\',\'city\');">'.$city_name[$v].'</a></li>';
                    }else{
	...
	function sign_action(){
	    ...
	    if($lastregdate!=$date){
	        ...
	        if($nid){
                $data=$Member->FetchRatingInfo_lie(array("uid"=>$this->uid));
                $data['vip_stime'] = time();
                $data['vip_etime'] = strtotime("+1 day");
                $this->obj->update_once("lietou_statis",$data,"uid=".$this->uid);
	        ...
	        $arr['integral']=iconv("gbk","UTF-8","成功获取当天VIP套餐");
	        $arr['signday']=$signday;
