...
class common{
  ...
  public $dirname="";
	...
	function common($tpl,$db,$def="",$model="index",$m="") {
		...
		require(APP_PATH.'app/public/reaction.class.php');
        ...
        $this->obr= new model($db1,$db_config1['def'],array('uid'=>$this->uid,'username'=>$this->username,'usertype'=>$this->usertype));
		...
		if($this->uid){
			...
			if($this->usertype == 2){
				...
				$jobApplyNum = $this->obj->DB_select_num('userid_job', "`com_id`={$this->uid} and display=1 and `is_browse`= 1 ");
				if($jobApplyNum > 0){
		...
		if($this->config['sy_iscsrf']!='2'){
			...
			if($_POST){
				if($_POST['pytoken']!=$_SESSION['pytoken']){
					$this->ACT_layer_msg("��Դ��ַ�Ƿ���",8,$this->config['sy_weburl']);
				}
				unset($_POST['pytoken']);
			}
	...
	function member() {
		...
		if(!$this->uid && !$this->username){
			...
			$this->ACT_msg($login,"���ȵ�¼");
		}else{
			$shell=$this->GET_user_shell($this->uid,$_COOKIE['shell']);
			if(!is_array($shell)){
				$this->ACT_msg("../index.php","����Ȩ�����������µ�¼");
			}else{
	...
	function send_message($uidarr=array(),$title="",$content="",$messagealert=false,$user="admin"){
		if(is_array($uidarr)){
			...
			if($messagealert){
				$this->ACT_layer_msg("���ͳɹ���",9,$_SERVER['HTTP_REFERER']);
			}else{
				return $insert_id;
			}
		}else{
			$this->ACT_layer_msg("�����������������",8,$_SERVER['HTTP_REFERER']);
		}
	}
	...
	function array_action($job_info,$array=array()){
		...
		if($job_info['minsalary']&&$job_info['maxsalary']){
			...
			$job_info['salary_info'] ="��".$job_info['minsalary']."����";
		}else{
			$job_info['salary_info'] ="����";
		}
    ...
    function department_cache(){
        include PLUS_PATH."/department.cache.php";
        $this->yunset("dpdata",$dpdata);
        $this->yunset("dpclass_name",$dpclass_name);
    }
 		...
 		if($this->config["sy_email_".$data["type"]]==1 && $data["email"]){
			...
			if($smtp){
				if($this->CheckRegEmail($data['email'])){ 
					...
					$this->ACT_layer_msg("�����ʽ����",8,$_SERVER['HTTP_REFERER'],2,$type);
				}
			}else{
				$this->ACT_layer_msg( "��û���������䣬����ϵ����Ա��",8,$_SERVER['HTTP_REFERER'],2,$type);
			}
		}
...
//		){
//			if(!$this->config["sy_msguser"] || !$this->config["sy_msgpw"] || !$this->config["sy_msgkey"]||$this->config['sy_msg_isopen']!='1'){
//				$this->ACT_layer_msg( "��û�����ö��ţ�����ϵ����Ա��",8,$_SERVER['HTTP_REFERER'],2,$type);
//			}else{
...
//		}
		...
		if(trim($re) =='1'){
		  ...
		  include(CONFIG_PATH."db.data.php");
			...
			if($arr_data['msgreturn'][$re]){
				return "����ʧ�ܣ�״̬��".$arr_data['msgreturn'][$re];
			}else{
				return "����ʧ�ܣ�״̬��".$re;
			}
		}
	...
	function picmsg($pic,$url,$type=""){
		$error = array("1"=>"�ļ�̫��","2"=>"�ļ����Ͳ���","3"=>"ͬ���ļ��Ѿ�����","4"=>"�ƶ��ļ�����,����uploadĿ¼Ȩ��");
		if($error[$pic]!=""){
			if($type=="ajax"){
				echo "{";
				echo				"url: '".$pic."',\n";
    ...
    function ACT_layer_msg($msg = "�����ѳɹ���", $st = 9,$url='',$tm = 2,$type='0'){
        if(is_array($msg)){
            foreach($msg as $k=>$v){
                $Html.='<div id="'.$k.'">'.$v.'</div>';
            }
	...
	function check_token(){
		if($this->config['sy_iscsrf']!='2'){
			if($_SESSION['pytoken']!=$_GET['pytoken'] || !$_SESSION['pytoken']){
				...
				$this->ACT_layer_msg("��Դ��ַ�Ƿ���",8,'index.php');
				exit();
			}
		}
	...
	function send_warning($type,$emailcoment=''){
		...
		if(empty($row)){
			...
			if($type=="1"){
				$emailcoment="�û�����".$this->username."������ְλ�����涨��Ŀ�������Ƿ�������";
				if($this->config['warning_addjob_type']=="1"){
					...
					$this->obj->DB_update_all("member","`status`='2',`lock_info`='����ְλ�����涨��Ŀ'","`uid`='".$this->uid."'");
					$this->send_msg_email(array("email"=>$member['email'],"uid"=>$this->uid,"name"=>$this->username,"lock_info"=>'����ְλ�����涨��Ŀ',"type"=>"lock"));
					$this->unset_cookie();
				}
			}elseif($type=="2"){
				$emailcoment="�û�����".$this->username."�����ؼ��������涨��Ŀ�������Ƿ�������";
				if($this->config['warning_downresume_type']=="1"){
					$this->obj->DB_update_all("company_job","`r_status`='2'","`uid`='".$this->uid."'");
					$this->obj->DB_update_all("company","`r_status`='2'","`uid`='".$this->uid."'");
					$this->obj->DB_update_all("member","`status`='2',`lock_info`='���ؼ��������涨��Ŀ'","`uid`='".$this->uid."'");
					$this->send_msg_email(array("email"=>$member['email'],"uid"=>$this->uid,"name"=>$this->username,"lock_info"=>'���ؼ��������涨��Ŀ',"type"=>"lock"));
					$this->unset_cookie();
				}
			}elseif($type=="3"){
				$emailcoment="�û�����".$this->username."���������������涨��Ŀ�������Ƿ�������";
				if($this->config['warning_addresume_type']=="1"){
			 		$this->obj->DB_update_all("member","`status`='2',`lock_info`='�������������涨��Ŀ'","`uid`='".$this->uid."'");
			 		$this->obj->DB_update_all("resume","`r_status`='2'","`uid`='".$this->uid."' ");
			 		$this->obj->DB_update_all("resume_expect","`r_status`='2'","`uid`='".$this->uid."' ");
					$this->send_msg_email(array("email"=>$member['email'],'uid'=>$this->uid,'name'=>$this->username,"lock_info"=>'�������������涨��Ŀ',"type"=>"lock"));
					$this->unset_cookie();
				}
			}elseif($type=="4"){
				$emailcoment="�û�����".$this->username."����ֵ�����涨�������Ƿ�������";
				if($this->config['warning_recharge_type']=="1"){
					$this->obj->DB_update_all("company_job","`r_status`='2'","`uid`='".$this->uid."'");
					$this->obj->DB_update_all("company","`r_status`='2'","`uid`='".$this->uid."'");
					$this->obj->DB_update_all("member","`status`='2',`lock_info`='��ֵ�����涨���'","`uid`='".$this->uid."'");
					$this->send_msg_email(array("email"=>$member['email'],"uid"=>$this->uid,"name"=>$this->username,"lock_info"=>'��ֵ�����涨���',"type"=>"lock"));
					$this->unset_cookie();
				}
	...
	public function get_admin_msg($url, $show = '�����ѳɹ���') {
		...
		<html xmlns="http://www.w3.org/1999/xhtml"><head>
				...
				<title>��Ϣ��ʾ Powered by PHPYUN_JOB!</title>
				<style>
				a,a:visited{
				...
				<div id="man_zone">
				  <table width="30%" border="0" align="center"  cellpadding="0" cellspacing="1" class="table" bgcolor="#dfdfdf" style="margin-top:100px;">
				    <tr>
				      <th height="25" align="center"><font style="font-size:12px;" color="#000">��Ϣ��ʾ</font></th>
				    </tr>
				    <tr>
  ...
  function admin_log($data){
	...
	function ACT_msg($url='', $msg = "�����ѳɹ���", $st = 8,$tm = 3) {
		if($url==''){
	...
	function admin_get_user_login($username,$password,$url='index.php') {
		...
		if($ps){
			...
			$this->ACT_layer_msg("��¼�ɹ���",9,$url);
			
		} else {
			$this->ACT_layer_msg("������û�����",8,$url);
		}
	}
	...
	function upuser_statis($order){
		if($order['order_state']!='2'){
			...
			if($status){
				...
				if($order['type']=='2'){
					$this->insert_company_pay($order['integral'],2,$order['uid'],"����".$this->config['integral_pricename'],1,2,true);
				}
			}
			return $status;
		}
			...
			if($this->config['sy_maturitytime']!=date("Ymd")){				
				...
				if($time>=$this->config['sy_maturityfrequency']){
					...
					if($num>0){ 
						...
						$emailtitle=$this->config['sy_webname']."-��Ա��������";
						$emailcoment=$this->config['sy_webname']."-��".$num."λ��ҵ��Ա".$this->config['sy_maturityday']."���ڽ�Ҫ���ڣ����¼��վ��̨�鿴��";
						$emailData['to'] = $this->config['sy_webemail'];
