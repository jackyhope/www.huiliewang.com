...
function PageNavResume($paramer,$get,$table,$where,$Purls,$table2="",$islt='0',$_smarty_tpl){

    global $db_config1,$config;
    $db = new mysql($db_config1['dbhost'], $db_config1['dbuser'], $db_config1['dbpass'], $db_config1['dbname'], ALL_PS, $db_config1['charset'],$db_config1['def']);
    $url=array();
    if($paramer['islt']){
        $islt=$paramer['islt'];
    }
...
//        echo $hy;exit();
		...
		if($paramer['m']=="resume"){
            ...
            $jobs_id1=$get['jobs_id1']?$get['jobs_id1']:0;
            $jobs_id2=$get['jobs_id2']?$get['jobs_id2']:0;
            $jobs_id3=$get['jobs_id3']?$get['jobs_id3']:0;
            $jobs_id1=$paramer['untype']=="jobs_id1"?0:($jobs_id[0]?$jobs_id[0]:$jobs_id1);
            $jobs_id2=$paramer['untype']=="jobs_id2"?0:($jobs_id[0]?$jobs_id[0]:$jobs_id2);
            $jobs_id3=$paramer['untype']=="jobs_id3"?0:($jobs_id[0]?$jobs_id[0]:$jobs_id3);
            $hebing[]=$jobs_id1;
            $hebing[]=$jobs_id2;
            $hebing[]=$jobs_id3;
