...
class index_controller extends common{
	...
	function RedLoginHead_action(){
		if($this->uid!=""&&$this->username!=""){
			...
			$login='<li><a href="'.$login_url.'">会员登陆</a></li>';
			...
			if($this->config['sy_qqlogin']=='1'||$this->config['sy_sinalogin']=='1'||$this->config['sy_wxlogin']=='1'){
				...
				if($_GET['type']=='index'){
					...
					if($this->config['sy_qqlogin']=='1'){
						$kjlogin.='<li><img src="'.$this->config['sy_weburl'].'/app/template/'.$this->config['style'].'/images/yun_qq.png" class="png" ><a href="'.$this->config['sy_weburl'].'/qqlogin.php'.'">QQ登陆</a></li>';
					}
					if($this->config['sy_sinalogin']=='1'){
						$kjlogin.='<li><img src="'.$this->config['sy_weburl'].'/app/template/'.$this->config['style'].'/images/yun_sina.png" class="png" ><a href="'.Url("sinaconnect",array(),"1").'">新浪登陆</a></li>';
					}
					if($this->config['sy_wxlogin']=='1'){
						$kjlogin.='<li><img src="'.$this->config['sy_weburl'].'/app/template/'.$this->config['style'].'/images/yun_wx.png" class="png" ><a href="'.Url("wxconnect",array(),"1").'">微信登陆</a></li>';
					}
				}else{
					$flogin='<div class="fastlogin fr">';
					if($this->config['sy_qqlogin']=='1'){
						$flogin.='<span style="width:80px;"><img src="'.$this->config['sy_weburl'].'/app/template/'.$this->config['style'].'/images/yun_qq.png" class="png" > <a href="'.$this->config['sy_weburl'].'/qqlogin.php'.'">QQ登陆</a></span>';
					}
					if($this->config['sy_sinalogin']=='1'){
	...
	function claim_action(){
		if((int)$_GET['uid']){
			...
			if($row['source']=="6" && $row['email']!=""){
				...
				$this->layer_msg('<div class="rl_box"><div class="rl_yx_p">已发送到您的邮箱：</div><div class="rl_yx">'.$newemail.'，</div><div class="">请登陆您的邮箱重置帐号密码！</div><div class="">如换邮箱请联系客服电话：</div><div class="rl_tel">'. $this->config['sy_freewebtel'] .'</div></div>',9,0);   
			}else{
	...
	function zphcom_action(){
		...
		if(!$this->uid || !$this->username || $this->usertype!=2){
			$arr['status']=0;
			$arr['content']=iconv("gbk","utf-8","您还没有登陆，<a href='javascript:void(0);' onclick=\"showlogin('2');\" style='color:#1d50a1'>请先登陆</a>！");
		}elseif(!$_GET['jobid']){
			$arr['status']=0;
	...
	function userreg_action(){
		...
		if($userid){
			...
			$this->get_integral_action($userid,"integral_login","会员登陆");
	...
	function footertj_action(){
		...
		if(!$this->uid){
			$html.='<a href="'.Url('login').'" class="tip_bottom_login fl">登陆</a>
