		...
		if($user){
			...
			if($this->uid && $this->username && ($this->usertype==2 || $this->usertype==3)){
				...
				if(is_array($row)){
					...
					$user['link_msg']="<a href='javascript:void(0)' onclick=\"for_link('{$user['id']}','".Url("ajax",array('c'=>'for_link'))."')\">查看联系方式</a>";
                                        $user['link_wapmsg']="<a href='javascript:void(0)' onclick=\"for_link('{$user['id']}','".Url("ajax",array('c'=>'forlink'))."')\">查看联系方式</a>";
