 ...
 */
class lietou_pay_controller extends common
{
	function set_search(){
	...
	function index_action(){
		...
		$rows=$this->get_page("lietou_pay",$where,$pageurl,$this->config['sy_listnum']);
		...
		if(is_array($rows)){
			foreach($rows as $k=>$v){
				...
				$lietou=$this->obj->DB_select_all("lietou","`uid` in(".pylode(',',$com).")","`uid`,`name`");
		}
		$info=array_merge($resume,$lietou);
		foreach($info as $v){
		...
		$this->yuntpl(array('admin/admin_lietou_pay'));
	}
	...
	function del_action(){
	    ...
	    if($_GET['del']){
	    	...
	    	if($del){
	    		if(is_array($del)){
					$this->obj->DB_delete_all("lietou_pay","`id` in(".@implode(',',$_GET['del']).")","");
					$del=@implode(',',$_GET['del']);
		    	}else{
		    		$this->obj->DB_delete_all("lietou_pay","`id`='$del'");
		    	}
	    ...
	    if(isset($_GET['id'])){
			$result=$this->obj->DB_delete_all("lietou_pay","`id`='".$_GET['id']."'" );
