 ...
 */
class admin_lietou_company_controller extends common{
	function set_search(){ 
	...
	function index_action(){
		...
		$rows=$this->get_page("fake_company",$where,$pageurl,$this->config['sy_listnum']);
		...
		$this->yuntpl(array('admin/admin_lie_company'));
	}
		...
		if($_POST['submit']){
			...
			if($username==""||strlen($username)<2||strlen($username)>15){
				...
				if(is_array($user)){
					...
					if($uid>0){
						$this->obj->DB_insert_once("lietou","`uid`='$uid',`name`='$name',`linkphone`='$areacode-$telphone-$exten',`linktel`='$moblie',`linkmail`='$email',`address`='$address'");
						$rat_arr = @explode("+",$rating_name);
						$value = "`uid`='$uid',";
			...
			if($_POST['type']){
				echo "<script type='text/javascript'>window.location.href='index.php?m=admin_lietou_job&c=show&uid=".$nid."'</script>";die;
			}else{
				if($data['type']=='9'){
	...
	function mapply_action(){
	    ...
	    $this->yunset("list",$list);
	    $this->yuntpl(array('admin/admin_lietou_mapply'));
	}
	...
	function mjob_action(){
	    ...
	    $pageurl=Url($_GET['m'],$urlarr,'admin');
	    $M=$this->MODEL();
	    $PageInfo=$M->get_page("lietou_job",'`uid`='.intval($_GET['comid']).'',$pageurl,$this->config['sy_listnum']);
	...
	function mintegral_action(){
	    ...
	    $this->yunset("list",$list);
	    $this->yuntpl(array('admin/admin_lietou_mintegral'));
	}
	...
	function morder_action(){
		...
		$this->yunset("rows",$rows);
		$this->yuntpl(array('admin/admin_lietou_morder'));
	}
	...
	function morderadd_action(){
	    ...
	    $this->yuntpl(array('admin/admin_lietou_morder_add'));
	}
