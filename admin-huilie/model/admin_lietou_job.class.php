 ...
 */
class admin_lietou_job_controller extends common{
	function set_search(){
	...
	function index_action(){
		...
		if(is_array($rows)){
            ...
            $useridjob = $this->obj->DB_select_all("userid_job","job_id in(".pylode(',',$jobids).") and display=1","count(id) as num,job_id");
            $msgnum = $this->obj->DB_select_all("userid_job","job_id in(".pylode(',',$jobids).") and display=1 and is_browse=6","count(id) as num,job_id");
...
//			$msgnum=$this->MODEL('resume')->GetUserMsgNums(array("`jobid` in(".pylode(',',$jobids).")"),array('field'=>"count(id) as num,`jobid`",'groupby'=>'jobid'));
		...
		$this->yuntpl(array('admin/admin_lietou_job'));
	}
		...
		if($_POST['update']){
			...
			if($_POST['id']&&$_POST['uid']==''){
				...
				if($id){
					...
					$this->ACT_layer_msg( "职位(ID:".$id.")发布成功！",9,'index.php?m=admin_lietou_job&c=show&uid='.$_POST['uid'],2,1);
				}else{
					$this->ACT_layer_msg( "职位发布失败！",8,'index.php?m=admin_lietou_job&c=show&uid='.$_POST['uid'],2,1);
				}
			}
		}
